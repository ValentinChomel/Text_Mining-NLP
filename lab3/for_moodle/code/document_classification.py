...
path_to_data = "../data/" # add your absolute path here
...
all_terms = [terms for sublist in terms_by_doc for terms in sublist] ### fill the gap ### hint: flatten 'terms_by_doc' (you may use a list comprehension)

# compute average number of terms
avg_len = sum(n_terms_per_doc)/len(n_terms_per_doc)

# unique terms
all_unique_terms = list(set(all_terms)) ### fill the gap ### hint: get unique elements of 'all_terms'
...
for counter,unique_term in enumerate(list(idf.keys())):
    ...
    df = sum([unique_term in terms for terms in terms_by_doc_sets])### fill the gap ### 
    ...
    # idf
    idf[unique_term] = math.log10(float(n_doc)/(1+df))### fill the gap ### hint: use math.log10 and refer to the beginning of Section 2 in the handout
    if counter % 1e3 == 0:
...
collection_degrees = dict(zip(c_g.vs['name'], c_g.strength())) ### fill the gap ### hint: build a dict where the keys are the names of the nodes in the collection graph and the values are their unweighted degrees
